...
namespace StorageCore.Controllers
{
    ...
    public class LogingController1 : ControllerBase
    {
        ...
        private readonly string _dbName = "DataBase.sqlite";

        public LogingController1(JwtHelp jwtHelper)
        {
        ...
        private void UpdateLastLoginTime(SQLiteConnection conn, string Id_employee)
        {
            ...
            using (var cmd = new SQLiteCommand(sql, conn))
            {
                cmd.Parameters.AddWithValue("@Id_employee", Id_employee); //Indica a que empleado le hara la actualizacion de Login
                cmd.ExecuteNonQuery();
            }
        }
